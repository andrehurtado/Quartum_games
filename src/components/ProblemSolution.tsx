import { useVisible } from '@/hooks/useVisible'
import { useLocale } from '@/contexts/LocaleContext'
import { Container } from '@/components/ui/Container'
import { SectionHeading } from '@/components/ui/SectionHeading'
import { Check, Minus } from 'lucide-react'

const PROBLEM_KEYS = ['problem1', 'problem2', 'problem3', 'problem4'] as const
const SOLUTION_KEYS = ['solution1', 'solution2', 'solution3'] as const

export function ProblemSolution() {
  const { ref, visible } = useVisible()
  const { t } = useLocale()

  return (
    <section
      id="problem"
      ref={ref as React.RefObject<HTMLDivElement>}
      className={`section-padding transition-all ${visible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-4 scale-[0.98]'}`}
      style={{ backgroundColor: 'var(--bg-elevated)', transitionDuration: '400ms', transitionTimingFunction: 'var(--ease-smooth)' }}
    >
      <Container narrow>
        <SectionHeading
          title={t('problemTitle')}
          subtitle={t('problemSubtitle')}
        />
        <div className="mt-12 grid sm:grid-cols-2 gap-10 lg:gap-16">
          <div>
            <h3
              className="text-sm font-semibold uppercase tracking-wider"
              style={{ color: 'var(--muted)' }}
            >
              {t('challengeLabel')}
            </h3>
            <ul className="mt-4 space-y-3" role="list">
              {PROBLEM_KEYS.map((key) => (
                <li key={key} className="flex gap-3 text-sm" style={{ color: 'var(--text-soft)' }}>
                  <Minus className="w-4 h-4 shrink-0 mt-0.5" style={{ color: 'var(--muted)' }} aria-hidden />
                  {t(key)}
                </li>
              ))}
            </ul>
          </div>
          <div>
            <h3
              className="text-sm font-semibold uppercase tracking-wider"
              style={{ color: 'var(--accent)' }}
            >
              {t('solutionLabel')}
            </h3>
            <ul className="mt-4 space-y-3" role="list">
              {SOLUTION_KEYS.map((key) => (
                <li key={key} className="flex gap-3 text-sm" style={{ color: 'var(--text-soft)' }}>
                  <Check className="w-4 h-4 shrink-0 mt-0.5" style={{ color: 'var(--accent)' }} aria-hidden />
                  {t(key)}
                </li>
              ))}
            </ul>
          </div>
        </div>
      </Container>
    </section>
  )
}
